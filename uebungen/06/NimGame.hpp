...
class NimGame {
   public:
	 ...
	 removal loses */
      NimGame(unsigned int _number_of_heaps, unsigned int _maxtake);

      /* return the number of heaps */
      unsigned int get_number_of_heaps() const;

      /* return maximal number of sticks which can be taken;
	 return 0, if there is no such limit */
      unsigned int get_maximal_take() const;
